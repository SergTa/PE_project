...
def test_run_App():
    ...
    assert at.markdown[1].value == "Я умею летать." 
        or at.markdown[1].value == "Не удалось осуществить перевод, пожалуйста, повторите попытку"# Сравнение результата перевода на русский
    
    at.text_area[0].set_value('Я умею летать').run(timeout=30) # Вставка в поле текста на русском
    at.button[0].click().run(timeout=30) # Нажатие на кнопку "перевести"
    assert at.markdown[1].value == "I know how to fly." 
        or at.markdown[1].value == "Не удалось осуществить перевод, пожалуйста, повторите попытку"# Сравнение результата перевода на английский
